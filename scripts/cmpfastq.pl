...
sub IndexFastq
{
	...
	#if($line =~ m/^@(.*)#.*/) #II modified to macht my fastq that don't have #
	#instead, we introduce \S that means "do not much white space" because after it there is the identifier of read (1 or 2) and tag
	if($line =~ m/^@(.\S*)/)
{
    $fastqIndex{$1} = $pos;
...
sub CompareFastq
{
    ...
    #II modified so that the output have the fastq extension to be detected by e.g. CLCGenomicsWorkbench
    open(F1COUT, ">$file1-common.fastq") or die("Could not write to file: $file1-common.fastq\n");
    open(F2COUT, ">$file2-common.fastq") or die("Could not write to file: $file2-common.fastq\n");
    open(F1UOUT, ">$file1-unique.fastq") or die("Could not write to file: $file1-unique.fastq\n");
    open(F2UOUT, ">$file2-unique.fastq") or die("Could not write to file: $file2-unique.fastq\n");
